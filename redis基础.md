...
![](C:\Users\HuQiaoDong\Desktop\笔记\images\image-20211115223850897.png)
...
```bash
# 给集合的某个成员（坐标）添加经度和纬度值
geoadd [key] [longitude] [latitude] [member] [longitude] [latitude] [member]...

# 获取集合的某个成员（坐标）经纬度
geopos [key] [member]

# 计算同个集合两个成员的地理位置（经纬差）并换算成指定单位（m:米,km:千米,mi:英里,ft:英尺）
...
统计不重复元素个数，2^64只占12kb内存0.8%的错误率

##### **Bitmap（位图）**

操作二进制位来记录。常用于统计用户活跃不活跃，登陆没登陆等
...
> Redis默认开启RDB，AOF需要手动开启，当两种模式同时开启时，优先使用AOF，AOF关闭时则使用RDB

**RDB（==R==edis ==D==ata==B==ase）**

在进行RDB时，Redis服务的父进程会fork出一个子进程，子进程将所有内存中的Redis存储数据全部写入到RDB临时文件中，这个过程不会影响到Redis父进程的服务，临时文件写入完成后，会替换掉上一次的正式RDB文件，此时子进程退出。Redis默认的持久化方式就是RDB，因此不需要额外在配置文件中进行配置。RDB模式下保存的文件是dump.rdb
...
在开启AOF时，Redis服务的父进程会将所有执行过的写操作命令（不记录读操作命令）记录下来形成文件，该文件只允许追加不允许修改，redis在启动时会自动读取该文件的所有写操作命令并全部执行一遍进行数据的重写来达到恢复数据的目的。

AOF模式下保存的文件是appendonly.aof文件



>  总结：RDB保存的是一份数据的快照，而AOF保存的是一份写命令的记录
